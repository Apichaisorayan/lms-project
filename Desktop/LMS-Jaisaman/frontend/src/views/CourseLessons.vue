...
const loadCourse = async () => {
  try {
    ...
    const response = await axios.get(`http://localhost:5000/api/courses/${route.params.id}`, {
...
const loadLessons = async () => {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`http://localhost:5000/api/courses/${route.params.id}/lessons`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const saveLesson = async () => {
  ...
  try {
    ...
    if (editingLesson.value) {
      await axios.put(`http://localhost:5000/api/lessons/${editingLesson.value.id}`, lessonForm.value, config)
      toast.success('บันทึกสำเร็จ!', 'แก้ไขบทเรียนเรียบร้อยแล้ว')
    } else {
      await axios.post(`http://localhost:5000/api/courses/${route.params.id}/lessons`, lessonForm.value, config)
...
const loadResources = async (lessonId) => {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`http://localhost:5000/api/lessons/${lessonId}/resources`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const handleDocumentUpload = async (event) => {
  ...
  try {
    ...
    await axios.post(`http://localhost:5000/api/lessons/${selectedLesson.value.id}/resources`, {
