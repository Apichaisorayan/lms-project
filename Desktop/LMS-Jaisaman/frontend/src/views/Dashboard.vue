        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-sm text-gray-500">คอร์สทั้งหมด</span>
            </div>
            <div class="text-3xl font-bold text-gray-800">{{ stats.totalCourses }}</div>
            <p class="text-sm text-green-600 mt-2">+12% จากเดือนที่แล้ว</p>
          </div>
          ...
          <div class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-sm text-gray-500">นักเรียน</span>
            </div>
            <div class="text-3xl font-bold text-gray-800">{{ stats.totalStudents }}</div>
            <p class="text-sm text-green-600 mt-2">+8% จากเดือนที่แล้ว</p>
          </div>
              ...
              <div class="p-5">
                ...
                <div class="mb-4">
                  <div class="flex items-center justify-between text-sm mb-2">
                    ...
                    <span class="font-semibold text-primary">{{ course.progress || 0 }}/{{ course._count?.lessons || 0 }} บทเรียน</span>
                  </div>
                ...
                <button
                  ...
                  class="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  <BookOpen :size="18" />
                  <span>{{ enrolling === course.id ? 'กำลังลงทะเบียน...' : 'ลงทะเบียนเรียน' }}</span>
                </button>
              </div>
            </div>
          </div>
...
const loadDashboardData = async () => {
  try {
    ...
    // Load courses
    const coursesResponse = await axios.get('http://localhost:5000/api/courses?published=true', {
      headers: { Authorization: `Bearer ${token}` }
    })
    ...
    // Show enrolled courses first, then others
    const enrolledCourses = allCourses.filter(c => c.isEnrolled).slice(0, 3)
    const otherCourses = allCourses.filter(c => !c.isEnrolled).slice(0, 3 - enrolledCourses.length)
    courses.value = [...enrolledCourses, ...otherCourses]
...
const calculateProgress = (course) => {
  if (!course._count?.lessons || course._count.lessons === 0) return 0
  const progress = course.progress || 0
  return Math.round((progress / course._count.lessons) * 100)
}
