...
const loadCourse = async () => {
  try {
    ...
    const isEnrolled = enrollmentsResponse.data.some(e => e.courseId === route.params.id)
...
const loadProgress = async () => {
  try {
    const token = localStorage.getItem('token')
    const response = await axios.get(`http://localhost:5000/api/courses/${route.params.id}/progress`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const markAsComplete = async () => {
  ...
  try {
    const token = localStorage.getItem('token')
    await axios.post(
      `http://localhost:5000/api/lessons/${currentLesson.value.id}/complete`,
      {},
      { headers: { Authorization: `Bearer ${token}` } }
    )
    await loadProgress()
