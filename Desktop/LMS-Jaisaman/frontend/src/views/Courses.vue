...
const saveCourse = async () => {
  ...
  try {
    ...
    if (editingCourse.value) {
      await axios.put(`http://localhost:5000/api/courses/${editingCourse.value.id}`, courseForm.value, config)
    } else {
      await axios.post('http://localhost:5000/api/courses', courseForm.value, config)
    }
