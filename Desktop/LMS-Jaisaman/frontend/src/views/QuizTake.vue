...
const startQuiz = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/quizzes/${route.params.quizId}/start`, {
      method: 'POST',
      headers: {
...
const handleSubmit = async (autoSubmit = false) => {
  ...
  try {
    ...
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:5000/api/attempts/${attempt.value.id}/submit`, {
      method: 'POST',
      headers: {
