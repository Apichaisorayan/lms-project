...
const loadQuizzes = async () => {
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/lessons/${route.params.lessonId}/quizzes`, {
...
const loadAttempts = async () => {
  try {
    ...
    for (const quiz of quizzes.value) {
      const response = await fetch(`http://localhost:5000/api/quizzes/${quiz.id}/attempts`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
